...
export async function listSentOrders(userId) {
  ...
  const orders = await db
    .collection("orders")
    .find(foreignKeyFilter("senderId", userId))
    .sort({ createdAt: -1 })
    .toArray();
  const results = [];
  ...
  for (const orderDocument of orders) {
    ...
    results.push({ ...order, card: presentGiftCard(cardDocument), recipient });
  }
...
export async function listSellerOrders(sellerId) {
  ...
  const orderDocuments = await db
    ...
    .sort({ createdAt: -1 })
    .toArray();
  const orders = orderDocuments.map(presentOrder);
