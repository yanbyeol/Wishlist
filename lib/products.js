...
export async function listProducts({
  ...
  if (sort === "popular" && products.length > 0) {
    ...
    products.sort((first, second) => {
      ...
      const firstId = normalizeId(first._id);
      const secondId = normalizeId(second._id);

      if (firstId === secondId) {
        return 0;
      }

      return secondId > firstId ? 1 : -1;
    });
  }
...
export async function listProductsBySeller(sellerId, statuses) {
  const db = await getDatabase();
  const products = await db
    .collection("products")
    .find({
      ...foreignKeyFilter("sellerId", sellerId),
      status: { $in: getSellerProductStatuses(statuses) },
    })
...
export async function updateProductOwned(productId, sellerId, fields) {
  ...
  if (typeof fields.imageUrl === "string") {
    changes.imageUrl = fields.imageUrl;
  }
