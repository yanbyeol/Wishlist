...
export async function createGroupGift({
  ...
  const amount = Number(initialContribution?.amount);

  if (Number.isNaN(parsedExpiresAt.getTime()) || parsedExpiresAt <= now) {
    throw new Error("모집 종료일을 확인해 주세요.");
  }

  if (!Number.isSafeInteger(amount) || amount <= 0 || amount > product.price) {
    throw new Error("첫 참여 금액을 확인해 주세요.");
  }

  const groupGiftId = new ObjectId();
  ...
  const reachedGoal = amount === product.price;
  const groupGift = {
    _id: groupGiftId,
    ...
    status: reachedGoal ? "funded" : "funding",
  ...
  const pendingContribution = {
    ...
    userId: normalizedOrganizerId,
    guestEmail: null,
    nickname: initialContribution.nickname,
    amount,
    message: initialContribution.message,
  ...
  const groupGifts = db.collection("groupGifts");

  await contributions.insertOne(pendingContribution);

  try {
    const paymentResult = await contributions.updateOne(
      { _id: contributionId, paymentStatus: "pending" },
      {
        $set: {
          paymentStatus: "paid",
          paymentId: `mock-contribution-${normalizeId(contributionId)}`,
          updatedAt: new Date(),
        },
      },
    );
    ...
    if (paymentResult.matchedCount !== 1) {
      throw new Error("첫 참여 결제를 완료하지 못했습니다.");
    }

    // 첫 참여 결제가 완료된 뒤에만 공동선물 기록을 생성합니다.
    await groupGifts.insertOne(groupGift);
  } catch (error) {
    await Promise.allSettled([
      contributions.deleteOne({ _id: contributionId }),
      groupGifts.deleteOne({ _id: groupGiftId }),
    ]);
    throw error;
  }
  ...
  if (reachedGoal) {
    order = await finishFundedGroupGift(normalizedGroupGiftId);
  }

  return {
    groupGift: presentGroupGift(groupGift),
    order,
  };
}
...
export async function findOpenGroupGiftForProduct(recipientId, productId) {
  ...
  if (
    groupGiftDocument?.status === "funding"
    && new Date(groupGiftDocument.expiresAt) <= new Date()
  ) {
    return getGroupGiftById(groupGiftDocument._id);
  }
...
async function settleExpiredGroupGift(db, groupGiftDocument) {
  if (
    groupGiftDocument?.status !== "funding"
    || new Date(groupGiftDocument.expiresAt) > new Date()
  ) {
    return groupGiftDocument;
  }

  const reachedGoal = Number(groupGiftDocument.currentAmount)
    >= Number(groupGiftDocument.targetAmount);
  const nextStatus = reachedGoal ? "funded" : "goal_not_met";
  const result = await db.collection("groupGifts").updateOne(
    { _id: groupGiftDocument._id, status: "funding" },
    { $set: { status: nextStatus, updatedAt: new Date() } },
  );
  let settledDocument = result.matchedCount === 1
    ? { ...groupGiftDocument, status: nextStatus }
    : await db.collection("groupGifts").findOne({ _id: groupGiftDocument._id });

  if (settledDocument?.status === "funded") {
    try {
      await finishFundedGroupGift(normalizeId(groupGiftDocument._id));
    } catch {
      // 완료 처리 실패 상태는 finishFundedGroupGift가 payment_failed로 저장합니다.
    }
    settledDocument = await db.collection("groupGifts").findOne({
      _id: groupGiftDocument._id,
    });
  }

  return settledDocument;
}

export async function getGroupGiftById(groupGiftId) {
  const db = await getDatabase();
  let initial = await db.collection("groupGifts").findOne(documentIdFilter(groupGiftId));
  ...
  if (initial.status === "funding" && new Date(initial.expiresAt) <= new Date()) {
    initial = await settleExpiredGroupGift(db, initial);
  }
...
export async function contributeToGroupGift({
  ...
  if (new Date(groupGiftDocument.expiresAt) <= new Date()) {
    await settleExpiredGroupGift(db, groupGiftDocument);
...
export async function extendGroupGift(groupGiftId, organizerId, nextExpiresAt) {
  const db = await getDatabase();
  let groupGiftDocument = await db
    .collection("groupGifts")
    .findOne(documentIdFilter(groupGiftId));

  if (!groupGiftDocument) {
    throw new Error("함께 선물하기 정보를 찾을 수 없습니다.");
  }

  if (normalizeId(groupGiftDocument.organizerId) !== normalizeId(organizerId)) {
    throw new Error("함께 선물하기를 만든 사용자만 기간을 연장할 수 있습니다.");
  }

  const now = new Date();
  if (groupGiftDocument.status === "funding" && new Date(groupGiftDocument.expiresAt) <= now) {
    groupGiftDocument = await settleExpiredGroupGift(db, groupGiftDocument);
  }

  if (!["funding", "goal_not_met"].includes(groupGiftDocument.status)) {
    throw new Error("현재 상태에서는 모집 기간을 연장할 수 없습니다.");
  }

  const parsedExpiresAt = new Date(nextExpiresAt);
  const currentExpiresAt = new Date(groupGiftDocument.expiresAt);

  if (Number.isNaN(parsedExpiresAt.getTime())) {
    throw new Error("새 종료일을 확인해 주세요.");
  }

  if (parsedExpiresAt <= currentExpiresAt || parsedExpiresAt <= now) {
    throw new Error("새 종료일은 기존 종료일보다 이후여야 합니다.");
  }

  const updated = await db.collection("groupGifts").findOneAndUpdate(
    {
      _id: groupGiftDocument._id,
      ...foreignKeyFilter("organizerId", organizerId),
      status: { $in: ["funding", "goal_not_met"] },
    },
    {
      $set: {
        status: "funding",
  ...
  if (!updated) {
    throw new Error("모집 기간을 연장하지 못했습니다. 새로고침 후 다시 시도해 주세요.");
  }

  return presentGroupGift(updated);
}

export async function cancelGroupGift(groupGiftId, organizerId) {
  const db = await getDatabase();
  let groupGiftDocument = await db
    .collection("groupGifts")
    .findOne(documentIdFilter(groupGiftId));
  ...
  if (normalizeId(groupGiftDocument.organizerId) !== normalizeId(organizerId)) {
    throw new Error("함께 선물하기를 만든 사용자만 모집을 종료할 수 있습니다.");
  }

  if (groupGiftDocument.status === "funding" && new Date(groupGiftDocument.expiresAt) <= new Date()) {
    groupGiftDocument = await settleExpiredGroupGift(db, groupGiftDocument);
  }

  if (groupGiftDocument.status !== "funding") {
    throw new Error("진행 중인 함께 선물하기만 종료할 수 있습니다.");
  }

  const otherActiveContributionFilter = {
    ...foreignKeyFilter("groupGiftId", groupGiftId),
    paymentStatus: { $in: ["pending", "paid"] },
    userId: { $nin: foreignKeyCandidates([organizerId]) },
  };
  const otherActiveContribution = await db.collection("contributions").findOne(
    otherActiveContributionFilter,
    { projection: { _id: 1 } },
  );

  if (otherActiveContribution) {
    throw new Error("다른 참여자가 있어 모집을 종료할 수 없습니다.");
  }
  ...
  const updated = await db.collection("groupGifts").findOneAndUpdate(
    {
      ...
      ...foreignKeyFilter("organizerId", organizerId),
      status: "funding",
      currentAmount: groupGiftDocument.currentAmount,
    },
    { $set: { status: "cancelled", updatedAt: new Date() } },
    { returnDocument: "after" },
  );
  ...
  if (!updated) {
    const contributionAddedWhileCancelling = await db.collection("contributions").findOne(
      otherActiveContributionFilter,
      { projection: { _id: 1 } },
    );

    if (contributionAddedWhileCancelling) {
      throw new Error("다른 참여자가 있어 모집을 종료할 수 없습니다.");
    }

    throw new Error("함께 선물하기를 종료하지 못했습니다. 새로고침 후 다시 시도해 주세요.");
  }
